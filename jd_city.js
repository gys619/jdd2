...
function uploadShareCode(code) {
  return new Promise(async resolve => {
    $.post({url: `http://${randomString(40)}.123/upload/city?code=${code}&ptpin=${encodeURIComponent(encodeURIComponent($.UserName))}`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(JSON.stringify(err))
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `http://${randomString(40)}.123/city`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
